...
**Architecture:** 复用现有 `CartQuantity` 小组件作为唯一代码演示对象。先修复一个现有边界 bug，用于 debugging 演示；再新增一个 `stock < 0` 负库存归零边界行为，用于 TDD 演示；最后更新中文演示文档和 prompt。
...
```js
  it('treats negative stock as unavailable', async () => {
    ...
    expect(increase.attributes('disabled')).toBe('')
...
```text
FAIL src/components/CartQuantity.test.js > CartQuantity > treats negative stock as unavailable
AssertionError: expected undefined to be ''
```

This confirms the new test catches missing negative-stock handling.
...
```js
const availableStock = computed(() => Math.max(0, props.stock))
const canIncrease = computed(() => props.quantity < availableStock.value)
```
...
```text
CartQuantity.vue contains availableStock and the canIncrease fix.
CartQuantity.test.js contains one new negative-stock test.
```
...
```md
**本项目 demo：**
在 `CartQuantity.test.js` 中先新增 `stock < 0` 的边界测试，确认测试失败后，再用最小实现把负库存按 `0` 处理。演示重点不是多写代码，而是让 Codex 先用测试说清楚“什么叫改对了”。
```
